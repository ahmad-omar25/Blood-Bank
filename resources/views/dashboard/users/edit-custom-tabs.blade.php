                ...
                @foreach($models as $index => $model)
                    <div class="tab-pane {{$index == 0 ? 'active' : ''}}" id="{{$model}}">
                        <div class="tabs_pre" style="padding: 15px">
                            ...
                            <label style="margin: 0 5px"><input style="margin: 0 10px" type="checkbox" name="permissions[]"
                                 {{$row->hasPermission($model. '_' .$map) ? 'checked' : ''}}
                                 value="{{$model}}_{{$map}}">{{__('control.' . $map)}}</label>
                            @endforeach
                        </div>
                    </div>
                @endforeach
